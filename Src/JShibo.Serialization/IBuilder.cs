    ...
    internal abstract class IBuilder
    {
        ...
        internal virtual Serialize<T> GenerateSerializationType<T>(Type type)
        {
            ...
            else if (type.IsClass)
            {
                ...
                //Console.WriteLine("SerializeFields " + w.ElapsedMilliseconds);
                SerializeFields<T>(type, mthdIL, tpmEvent);
        ...
        internal virtual Serialize<T> GenerateSizeSerializationType<T>(Type type)
        {
            ...
            DynamicMethod dynamicGet = new DynamicMethod("SizeSerialization_" + type.Name, typeof(void), new Type[] { typeof(T), typeof(object) }, typeof(object), true);//new DynamicMethod("DynamicGet", typeof(object), new Type[] { typeof(object) }, type, true);
            ILGenerator mthdIL = dynamicGet.GetILGenerator();
        ...
        internal virtual bool IsSizeType(Type type)
        {
            return !type.IsPrimitive;
        }
        ...
        private void SerializeValuePropertys<T>(Type type, ILGenerator mthdIL, LocalBuilder tpmEvent)
        {
        ...
        private void SerializeFields<T>(Type type, ILGenerator mthdIL, LocalBuilder tpmEvent)
        {
            //字段
            foreach (FieldInfo info in type.GetFields())
            {
        ...
        private void SerializePropertys<T>(Type type, ILGenerator mthdIL, LocalBuilder tpmEvent)
        {
        ...
        internal virtual void SerializeSizeFields<T>(Type type, ILGenerator mthdIL, LocalBuilder tpmEvent)
        {
            ...
            foreach (FieldInfo info in type.GetFields())
            {
                ...
                if (IsSizeType(ctype) == false)
                    continue;

                MethodInfo brWrite = CreateWriteSizeMethod<T>(ctype);
                mthdIL.Emit(OpCodes.Ldarg_0);//PU binary writer
                mthdIL.Emit(OpCodes.Ldloc, tpmEvent);//PU binary writer
        ...
        private void DeserializeFields<T>(Type type, ILGenerator deserializeIL, LocalBuilder tpmRetEvent, LocalBuilder tpmRetEvent2)
        {
            foreach (FieldInfo fi in type.GetFields())
            {
                if (fi.FieldType == typeof(string[]))
        ...
        private void DeserializePropertys<T>(Type type, ILGenerator deserializeIL, LocalBuilder tpmRetEvent, LocalBuilder tpmRetEvent2)
        {
            foreach (PropertyInfo pi in type.GetProperties())
            {
                if (pi.PropertyType == typeof(string[]))
        ...
        internal virtual void DeserializeBaseType<T>(Type type, ILGenerator deserializeIL)
        {
        ...
        private void CutTail<T>(ILGenerator mthdIL)
        {
            ////该写的值会被写入，但状态不会被记录(最终版)
